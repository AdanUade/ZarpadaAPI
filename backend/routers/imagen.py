...
async def probar_prenda(
    ...
    buf_result = BytesIO()
    img_result.save(buf_result, "JPEG")
    buf_result.seek(0)
    cloudinary_url = upload_image_to_cloudinary(buf_result.getvalue())
    ...
    if len(historial) >= 5:
        antiguo_url = historial.pop(0)

    historial.append(cloudinary_url)
